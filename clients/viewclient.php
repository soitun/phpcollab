...
try {
    $clientId = filter_var((int)$request->query->get("id"), FILTER_VALIDATE_INT);
    ...
    if ($clientsFilter == "true" && $session->get("profile") == "2") {
        ...
        if (count($memberTest) == "0") {
            ...
            $clientDetail = $orgs->getOrganizationById($clientId);
        }
    } elseif ($clientsFilter == "true" && $session->get("profile") == "1") {
        $clientDetail = $orgs->getOrganizationByIdAndOwner($clientId, $session->get("id"));
    } else {
        $clientDetail = $orgs->getOrganizationById($clientId);
    }
    ...
    $block1->contentRow($strings["url"],
        $blockPage->buildLink($clientDetail['org_url'], $clientDetail['org_url'], 'out'));
    ...
    if (file_exists("../logos_clients/" . $clientId . "." . $clientDetail['org_extension_logo'])) {
        $block1->contentRow($strings["logo"],
            '<div class="logoContainer"><img alt="' . $clientDetail['org_name'] . '" src="../logos_clients/' . $clientId . '.' . $clientDetail['org_extension_logo'] . '"></div>');
    }
    ...
    if ($session->get("profile") == "0" || $session->get("profile") == "1") {
        ...
        $block1->paletteScript(1, "edit", "../clients/editclient.php?id=" . $clientDetail['org_id'] . "",
            "true,true,false",
            $strings["edit"]);
    ...
    $block2->form = "clPr";
    $block2->openForm("../clients/viewclient.php?id=" . $clientId . "#" . $block2->form . "Anchor",
        null,
        $csrfHandler);
...
//if mantis bug tracker enabled
    ...
    $block2->sorting("organization_projects", $sortingUser["organization_projects"], "pro.name ASC",
        $sortingFields = array(
            0 => "pro.id",
            1 => "pro.name",
            2 => "pro.priority",
...
//if mantis bug tracker enabled
    ...
    $block3->form = "clU";
    $block3->openForm("../clients/viewclient.php?id=" . $clientId . "#" . $block3->form . "Anchor",
        null,
        $csrfHandler);
    ...
    $listMembers = $members->getMembersByOrg($clientId, $block3->sortingValue);
    ...
    if ($listMembers) {
        ...
        foreach ($listMembers as $member) {
            ...
            $block3->cellRow($blockPage->buildLink("../users/viewclientuser.php?id=" . $member['mem_id'] . "&organization=" . $clientId,
                $member['mem_name'], 'in'));
    ...
    if ($session->get("profile") == "0" || $session->get("profile") == "1") {
        $block3->paletteScript(0, "add", "../users/addclientuser.php?organization=" . $clientId,
            "true,true,true", $strings["add"]);
        $block3->paletteScript(1, "remove", "../users/deleteclientusers.php?orgid=" . $clientId,
            "false,true,true", $strings["delete"]);
    }
    $block3->paletteScript(2, "info", "../users/viewclientuser.php?organization=" . $clientId,
        "false,true,false", $strings["view"]);
    if ($session->get("profile") == "0" || $session->get("profile") == "1") {
        $block3->paletteScript(3, "edit", "../users/updateclientuser.php?orgid=" . $clientId,
